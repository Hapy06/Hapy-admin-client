...
function Hote_Tables(props:PropsType) {
    ...
    const listTables = [
        ...
        {number:8, state: 'free'},
        {number:9, state: 'free'},
        {number:10, state: 'full'},
        {number:11, state: 'free'},
        {number:12, state: 'free'},
        {number:13, state: 'free'},
        {number:14, state: 'free'},
        {number:15, state: 'free'},
        {number:16, state: 'full'},
    ] ;
    ...
    const listReservations = [
        ...
        {number:8, state: 'free'},
        {number:9, state: 'free'},
        {number:10, state: 'free'},
        {number:11, state: 'free'},
        {number:12, state: 'full'},
        {number:13, state: 'free'},
        {number:14, state: 'full'},
        {number:15, state: 'free'},
        {number:16, state: 'free'},
    ] ;
    ...
    return (
        <>
            <Hote_Top classAdditional={blurBG} handleSwitchListToShow={(value)=>setListToShow(value)}
                      showLeftBtn={listToShow == 'Reservations'} leftBtnComponent={<HapyButtonWithIcon text='Ajouter une réservation'
                                                                                   handleClick={()=>handleOpenModal(<HoteModalAddReservation containerStyle={{marginTop:150}} handleCloseModal={handleCloseModal}/>)}
                        ...
                        <div className="row">
                            { listTables.map( (item, index) => (
                                <HapyHoteTableItem marginLeft={49} marginBottom={49} state={item.state}
                                                   handleClick={()=>handleOpenModal(<HoteModalOpenTable containerStyle={{marginTop:50}} handleCloseModal={handleCloseModal}/> )} number={item.number} btnWidth={64} />
                            )) }
                        </div>
                    ) : (
                        <div className="row">
                            { listReservations.map( (item, index) => (
                                <HapyHoteTableItem marginLeft={49} marginBottom={49} isChecked={item.state == 'full'}
                                                   handleClick={()=>handleOpenModal(<HoteModalDetailReservation containerStyle={{marginTop:100}} handleCloseModal={handleCloseModal}/>)} number={item.number} btnWidth={64} />
                            )) }
                        </div>
                    ) }
                    <div className="text-center fixed-bottom mb-4 center-force" style={{width:1194}}>
                        <span className="float-start"><IconArrowLeft/></span>
                        <span className="float-none fw-6">Terrasse Droite</span>
                        <span className="float-end"><IconArrowRight/></span>
                    </div>
                </div>
                { isModalOpened.state && (
