...
function Preparation_Attente(props: PropsType) {
  ...
  const handleClickPause = () => {
    ...
    if (temp.orderDetail.status == "cooking") {
      ...
      if (temp.orderCooking) {
        ...
        temp.listPausedOrders = temp.listPausedOrders.filter(
          (order) => order.id != temp.orderDetail.id
        );
  ...
  const handleClickValidate = () => {
    ...
    putRequest(
      API_REQUEST_ORDER + "/updateWithCouponsReadyIds",
      temp.orderDetail.id,
      { couponsReadyIds: temp.orderDetail.couponsReadyIds },
      (response) => {
  ...
  const showErrorFunction = (
    errorMessage: string,
    color: "text-success" | "text-danger" = "text-success",
    timeout: number = 5000
  ) => {
          ...
          <div className="row">
            <div className="col-lg-3 col-md-4 mt-5">
              <PreparationCommandBox
                order={preparationProcess.orderDetail}
                borderOrange={true}
