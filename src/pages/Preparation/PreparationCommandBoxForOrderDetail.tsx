...
function PreparationCommandBoxForOrderDetail(props: PropsType) {
  ...
  const [couponsReadyIds, setCouponsReadyIds] = useState<string[]>(
    props.order?.couponsReadyIds || []
  );
  ...
  return (
    <>
      <div
        ...
        style={{ border: props.borderOrange ? "2px solid #F7B927" : "" }}
      >
        <div className="text-disabled row">
          <div className="f-12 col-3 mt-2 fw-3">{props.order.startTime}</div>
        ...
        <div className="row mt-3">
          ...
          <div
            className="col-lg-4 col-md-4 text-end"
            style={{ marginLeft: -5 }}
          >
        ...
        <br />
        <br />
        {props.order.coupons.map(
          (coupon: Coupon, index: number) =>
            /*coupon.cookingStation.toLowerCase().includes('cuisine')*/ coupon.cookingStation !=
              "Bar" && (
              <div
                key={coupon.id}
                className={
                  coupon.isPregnant ? "row fw-5 mb-3 mt-4" : "row fw-5 mb-3"
                }
              >
                ...
                {couponsReadyIds.includes(coupon.id) && (
                  <span
                    ...
                    className="col-2"
                    onClick={() => handleSelectedCoupon(coupon.id)}
                  >
                ...
                <span className="col-10" style={{ marginTop: -20 }}>
                  ...
                  {coupon.isPregnant ? (
                    <span className="text-orange">
                      ...
                      <span className="fw-5 f-16" style={{ marginLeft: 10 }}>
                        {coupon.productVariant.name}
                      </span>
                    </span>
                  ) : (
                    <span className="fw-5 f-16" style={{ marginLeft: 15 }}>
                      {coupon.productVariant.name}
                    </span>
                  )}
                  {coupon.ingredientsModifiablesStates.length > 0 && (
                    <div style={{ fontSize: 12, marginLeft: 15 }}>
                      {coupon.ingredientsModifiablesStates.map(
                        (elt: string, index) => (
                          <span key={index} className="fw-5">
                            {elt}
                            <br />
                          </span>
                        )
