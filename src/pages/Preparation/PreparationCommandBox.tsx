...
function PreparationCommandBox(props:PropsType) {
    ...
    useEffect(() => {
        props.order.coupons.map((coupon: any)=>coupon?.cookingStation?.includes('Cuisine')?setCouponNumber(prevState=>prevState+1): null)
        return ()=>console.log(timer.timerText)
    }, []) ;
