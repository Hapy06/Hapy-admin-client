...
const Index = ({ index, coupon }) => {
  if (
    /*coupon.cookingStation == getAdminProcessValues('userLogged').position*/ true
  ) {
    return (
      <div
        key={index}
        ...
        <span className="col-10" style={{ marginTop: -24 }}>
          ...
          {coupon.isPregnant ? (
            <span className="text-orange">
              ...
              <span className="fw-5 f-16" style={{ marginLeft: 10 }}>
                {coupon.productVariant?.name || "Inconnue"}{" "}
                {/*<span style={{fontSize:12}}>({coupon.cookingStation})</span>*/}
              </span>
            </span>
          ) : (
          ...
          {coupon.ingredientsModifiablesStates?.length > 0 && (
            <div style={{ fontSize: 12, marginLeft: 15 }}>
              {coupon.ingredientsModifiablesStates
                .filter((elt) => elt.includes("sans"))
                .map((elt: string, index) => (
